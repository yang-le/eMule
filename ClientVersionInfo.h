...
public:
	explicit CClientVersionInfo(CString strPCEncodedVersion)
	{
		...
		CString strClientType = strPCEncodedVersion.Left(posSeparator).Trim();
		CString strVersionNumber = strPCEncodedVersion.Mid(posSeparator+1).Trim();
	...
	{
		return ((c1.m_nVerMajor == (UINT)(-1) || c2.m_nVerMajor == (UINT)(-1) || c1.m_nVerMajor == c2.m_nVerMajor)
			&& (c1.m_nVerMinor == (UINT)(-1) || c2.m_nVerMinor == (UINT)(-1) || c1.m_nVerMinor == c2.m_nVerMinor)
			&& (c1.m_nVerUpdate == (UINT)(-1) || c2.m_nVerUpdate == (UINT)(-1) || c1.m_nVerUpdate == c2.m_nVerUpdate)
			&& (c1.m_nVerBuild == (UINT)(-1) || c2.m_nVerBuild == (UINT)(-1) || c1.m_nVerBuild == c2.m_nVerBuild)
			&& (c1.m_ClientTypeMajor == (UINT)(-1) || c2.m_ClientTypeMajor == (UINT)(-1) || c1.m_ClientTypeMajor == c2.m_ClientTypeMajor)
			&& (c1.m_ClientTypeMinor == (UINT)(-1) || c2.m_ClientTypeMinor == (UINT)(-1) || c1.m_ClientTypeMinor == c2.m_ClientTypeMinor)
			);
	}
	...
	friend bool operator >(const CClientVersionInfo& c1, const CClientVersionInfo& c2)
	{
		if ((c1.m_ClientTypeMajor == (UINT)(-1) || c2.m_ClientTypeMajor == (UINT)(-1) || c1.m_ClientTypeMajor != c2.m_ClientTypeMajor)
			|| (c1.m_ClientTypeMinor != c2.m_ClientTypeMinor))
			return false;
		if (c1.m_nVerMajor != (UINT)(-1) && c2.m_nVerMajor != (UINT)(-1) && c1.m_nVerMajor > c2.m_nVerMajor)
			return true;
		if (c1.m_nVerMinor != (UINT)(-1) && c2.m_nVerMinor != (UINT)(-1) && c1.m_nVerMinor > c2.m_nVerMinor)
			return true;
		if (c1.m_nVerUpdate != (UINT)(-1) && c2.m_nVerUpdate != (UINT)(-1) && c1.m_nVerUpdate > c2.m_nVerUpdate)
			return true;
		if (c1.m_nVerBuild != (UINT)(-1) && c2.m_nVerBuild != (UINT)(-1) && c1.m_nVerBuild > c2.m_nVerBuild)
			return true;
		return false;
	}
...
private:
	void init(uint32 nVerMajor, uint32 nVerMinor, uint32 nVerUpdate, uint32 nVerBuild, uint32 ClientTypeMajor, uint32 ClientTypeMinor) {
