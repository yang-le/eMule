...
public:
	...
	void* Open(LPCTSTR File)
	{
		if (m_pfnMediaInfo4_Open)
			return (*m_pfnMediaInfo4_Open)((LPSTR)(LPCSTR)CStringA(File));
...
bool CGetMediaInfoThread::GetMediaInfo(HWND hWndOwner, const CShareableFile* pFile, SMediaInfo* mi, bool bSingleFile)
{
 	...
 	if (theApp.GetProfileInt(_T("eMule"), _T("MediaInfo_WM"), 1))
	{
		try
		{
			...
			{
				if (bIsWM && (   szExt.Compare(_T(".asf")) != 0
							  && szExt.Compare(_T(".wm")) != 0
							  && szExt.Compare(_T(".wma")) != 0
							  && szExt.Compare(_T(".wmv")) != 0
							  && szExt.Compare(_T(".dvr-ms")) != 0))
...
//no vbr bitrate					mi->strInfo << _T("   ") << GetResString(IDS_BITRATE) << _T(":\t") << ((mp3info->vbr_bitrate ? mp3info->vbr_bitrate : mp3info->bitrate) + 500) / 1000 << _T(" ") << GetResString(IDS_KBITSSEC) << _T("\n");
				...
				mi->iAudioStreams++;
//no vbr bitrate				mi->audio.nAvgBytesPerSec = mp3info->vbr_bitrate ? mp3info->vbr_bitrate/8 : mp3info->bitrate/8;
		...
		|| (eFileType == ED2KFT_AUDIO || eFileType == ED2KFT_VIDEO))
	{
					...
					if (Handle)
					{
						...
						if (szExt[0] == _T('.') && szExt[1] != _T('\0'))
						{
							...
							if (!str.IsEmpty())
							{
								...
								{
									if (strFmtExt.Compare((LPCTSTR)szExt + 1) == 0)
									{
										bFoundExt = true;
										break;
									}
						...
						if (iVideoStreams > 0)
						{
							...
							if (str.CompareNoCase(_T("VBR")) == 0) {
								...
								if (iBitrate == -1)
									uAllBitrates = (UINT)-1;
								else //if (uAllBitrates != (UINT)-1) always true
		...
		if (theApp.GetProfileInt(_T("eMule"), _T("MediaInfo_MediaDet"), 1)
			&& (   thePrefs.GetInspectAllFileTypes()
			    || (szExt.Compare(_T(".ogm"))!=0 && szExt.Compare(_T(".ogg"))!=0 && szExt.Compare(_T(".mkv"))!=0)))
		{
			try
			{
