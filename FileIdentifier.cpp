...
bool CFileIdentifier::CalculateMD4HashByHashSet(bool bVerifyOnly, bool bDeleteOnVerifyFail)
{
	...
	uchar aucResult[16];
	CKnownFile::CreateHash(buffer, (uint32)m_aMD4HashSet.GetCount()*16u, aucResult);
	delete[] buffer;
	if (bVerifyOnly) {
...
bool CFileIdentifier::VerifyAICHHashSet()
{
	...
	for (uint32 nPart = 0; nPart < uPartCount; nPart++)
	{
		...
		uint32 nPartSize = (uint32)mini(PARTSIZE, (uint64)GetFileSize()-nPartStartPos);
...
bool CFileIdentifierSA::ReadIdentifier(CFileDataIO* pFile, bool bKadValidWithoutMd4)
{
	...
	if (!bSize)
		DebugLogWarning(_T("Size not included on reading fileidentifier"));
