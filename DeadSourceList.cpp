...
CDeadSource::CDeadSource(uint32 dwID, uint16 nPort, uint32 dwServerIP, uint16 nKadPort)
	: m_dwID(dwID), m_dwServerIP(dwServerIP), m_nPort(nPort), m_nKadPort(nKadPort), m_aucHash()
{
}

CDeadSource::CDeadSource(const uchar* paucHash)
	: m_dwID(0), m_dwServerIP(0), m_nPort(0), m_nKadPort(0)
{
...
bool operator==(const CDeadSource& ds1,const CDeadSource& ds2){
	...
	return (
		...
		// lowid kad check
		|| ( IsLowID(ds1.m_dwID) && !isnulmd4(ds1.m_aucHash) && md4cmp(ds1.m_aucHash, ds2.m_aucHash) == 0) );
}
...
CDeadSourceList::CDeadSourceList()
	: m_dwLastCleanUp(0), m_bGlobalList(false)
{
}
