...
//	m_aUDPServerRecords.RemoveAll(); will be called in next destructor
}
...
void CSearchList::NewSearch(CSearchListCtrl* pWnd, const CStringA& strResultFileType, uint32 nSearchID, ESearchType eSearchType, CString strSearchExpression, bool bMobilMuleSearch)
{
	...
	if (strSearchExpression.Find(_T("related:"), 0) != 0){ // ignore special searches
		...
		while (!strToken.IsEmpty()){
			if (strToken != "and" && strToken != "or" && strToken != "not")
...
void CSearchList::GetWebList(CQArray<SearchFileStruct, SearchFileStruct> *SearchFileArray, int iSortBy) const
{
	for (POSITION pos = m_listFileLists.GetHeadPosition(); pos != NULL;)
	{
		...
		for(POSITION pos2 = listCur->m_listSearchFiles.GetHeadPosition(); pos2 != NULL;)
		{
			...
			if (pFile == NULL || pFile->GetListParent() != NULL || pFile->GetFileSize() == 0ull || pFile->GetFileName().IsEmpty() || pFile->m_flags.noshow)
				continue;
...
bool CSearchList::AddToList(CSearchFile* toadd, bool bClientResponse, uint32 dwFromUDPServerIP)
{
	...
	// Spamfilter: Calculate the filename without any used keywords (and separators) for later use
	...
	for (POSITION pos = list->GetHeadPosition(); pos != NULL;)
	{
			...
			&& md4cmp(parent->GetFileHash(), toadd->GetFileHash()) == 0)
		{
			...
			for (POSITION pos2 = list->GetHeadPosition(); pos2 != NULL;) {
				...
				if (child->GetListParent() == parent)
				{
					...
					if (parent->IsKademlia())
					{
						...
						if (u != 0) {
							nPublishInfoTags++;
							uDifferentNames = maxi(uDifferentNames, ((u & 0xFF000000) >> 24));
							uPublishersKnown = maxi(uPublishersKnown, ((u & 0x00FF0000) >> 16));
			...
			if (bestEntry)
			{
				...
				parent->SetSourceCount(uAllChildsSourceCount);
				parent->SetCompleteSourceCount(uAllChildsCompleteSourceCount);
...
uint32 CSearchList::GetSpamFilenameRatings(const CSearchFile* pSearchFile, bool bMarkAsNoSpam){
	...
	if (!m_astrKnownSimilarSpamNames.IsEmpty() && !pSearchFile->GetNameWithoutKeyword().IsEmpty()){
		...
		for (int i = 0; i < m_astrKnownSimilarSpamNames.GetCount(); i++){
			bool bRemove = false;
			if (cname.Compare(m_astrKnownSimilarSpamNames[i]) == 0) {
				if (!bMarkAsNoSpam){
					if (cname.GetLength() <= 10)
			...
			for (POSITION pos = m_listFileLists.GetHeadPosition(); pos != NULL;) {
				...
				if (pParams != NULL){
					...
					for (POSITION pos2 = pSl->m_listSearchFiles.GetHeadPosition(); pos2 != NULL;)
						if (!pSl->m_listSearchFiles.GetNext(pos2)->m_flags.nowrite)
							++uCount;
...
void CSearchList::LoadSearches()
{
	...
	try {
		...
		uint32 nHighestKadSearchID = 0xFFFFFFFFu;
		uint32 nHighestEd2kSearchID = 0xFFFFFFFFu;
		for (unsigned nCount = (unsigned)file.ReadUInt16(); nCount > 0; --nCount) {
			...
			else if (pParams->eType == SearchTypeEd2kGlobal && pParams->dwSearchID < 0x80000000u)
				pParams->eType = SearchTypeKademlia;

			if (pParams->eType == SearchTypeKademlia && (nHighestKadSearchID == 0xFFFFFFFFu || nHighestKadSearchID < pParams->dwSearchID)) {
//				ASSERT( pParams->dwSearchID < 0x80000000u );
				nHighestKadSearchID = pParams->dwSearchID;
			} else if (pParams->eType != SearchTypeKademlia && (nHighestEd2kSearchID == 0xFFFFFFFFu || nHighestEd2kSearchID < pParams->dwSearchID)) {
//				ASSERT( pParams->dwSearchID >= 0x80000000u );
