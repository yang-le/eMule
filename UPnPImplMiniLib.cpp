...
void CUPnPImplMiniLib::StopAsyncFind()
{
	...
	if (!lockTest.Lock(MIN2MS(7))) {	// give the thread 7 seconds to exit gracefully - it should never really take that long
...
void CUPnPImplMiniLib::StartDiscovery(uint16 nTCPPort, uint16 nUDPPort, uint16 nTCPWebPort)
{
	...
	DebugLog(_T("miniupnpc (c) 2006-2017 Thomas Bernard - http://miniupnp.free.fr/"));
	m_nOldUDPPort = (ArePortsForwarded() == TRIS_TRUE) ? m_nUDPPort : 0;
	m_nUDPPort = nUDPPort;
	m_nOldTCPPort = (ArePortsForwarded() == TRIS_TRUE) ? m_nTCPPort : 0;
	m_nTCPPort = nTCPPort;
...
{
	static const char achTCP[] = "TCP";
	static const char achUDP[] = "UDP";
