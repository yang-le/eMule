...
int StartSSL()
{
	...
	int ret = mbedtls_x509_crt_parse_file(&srvcert, (const char*)thePrefs.GetWebCertPath().GetString());
	if (!ret) {
		mbedtls_pk_init(&pkey);
		ret = mbedtls_pk_parse_keyfile(&pkey, (const char*)thePrefs.GetWebKeyPath().GetString(), NULL);
		if (!ret) {
