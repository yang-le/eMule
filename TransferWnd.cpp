...
CTransferWnd::CTransferWnd(CWnd* /*pParent =NULL*/)
	: CResizableFormView(CTransferWnd::IDD), downloadlistactive(false), rightclickindex(0)
	, m_nDragIndex(0), m_nDropIndex(0), m_isetcatmenu(0), m_bIsDragging(false)
{
...
{
	TCHAR buffer[100];
	_sntprintf(buffer, _countof(buffer), _T("%u (%u ") + GetResString(IDS_BANNED).MakeLower() + _T(')'), number, (unsigned)theApp.clientlist->GetBannedCount());
...
void CTransferWnd::UpdateTabToolTips(int tab)
{
	if (tab == -1)
	{
		for (int i = 0; i < m_tooltipCats->GetToolCount();)
