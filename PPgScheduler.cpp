...
void CPPgScheduler::OnNmClickList(NMHDR*, LRESULT*)
{
	if (m_list.GetSelectionMark() > -1)
...
void CPPgScheduler::OnBnClickedRemove()
{
	...
	if (index != -1)
		theApp.scheduler->RemoveSchedule(index);
	FillScheduleList();
...
void CPPgScheduler::OnNmDblClkActionlist(NMHDR*, LRESULT *pResult)
{
...
void CPPgScheduler::OnNmRClickActionlist(NMHDR*, LRESULT *pResult)
{
	POINT point;
	::GetCursorPos(&point);
...
BOOL CPPgScheduler::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	if (item != -1)
		if (wParam == MP_CAT_EDIT) {
			InputBox inputbox;
			CString prompt;
			int action = (int)m_actions.GetItemData(item);
			...
			if (!inputbox.WasCancelled())
				m_actions.SetItemText(item, 1, inputbox.GetInput());
